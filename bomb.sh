#!/usr/bin/env bash
#set -x
#By suman kumar ~Drexploit02 github:- https://github.com/Drexploit02
...
echo
echo
printf "             ${S2}[${R0} Dr_Exploit${S2}] ${S3}Enter target with(91)  >>> ${R0}"
read targett
passd=$(echo "$targett" | md5sum | awk '{printf $1}')
if [[ $passd == $no1 || $passd == $no2 || $passd == $no3 || $passd == $mo1 || $passd == $mo2 || $passd == $mo3 ]]; then
echo

printf "${S2}[${S1}!${S2}]${S1}Wrong number lol ;)! its developer here!! Don't be so smart rotor. you can be self bombed for this wrong attempt!!Me Suman kumar ~Drexploit02 github:- https://github.com/Drexploit02/ ${R0}\n"
