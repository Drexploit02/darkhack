#!/usr/bin/env bash
#set -x
#By Ashi ~Drexploit02 github:- https://github.com/Drexploit02
...
#<<<---------Internet check--------->>>
INTERNET() {
  python assets/load.py -H checking-internet -P Drexploit02
...
echo
echo -e "       ${S6}       INSTAGRAM    :-   https://instagram.com/dr_exploit2.0" | pv -qL 10
echo
echo -e "       ${S6}       YOUTUBE         :-   https://youtube.com/DrExploit" | pv -qL 10
echo
...
echo
echo
printf "             ${S2}[${R0} Dr_Exploit${S2}] ${S3}Enter target number without(+91)  >>> ${R0}"
read targett
passd=$(echo "$targett" | md5sum | awk '{printf $1}')
if [[ $passd == $no1 || $passd == $no2 || $passd == $no3 || $passd == $mo1 || $passd == $mo2 || $passd == $mo3 ]]; then
echo

printf "${S2}[${S1}!${S2}]${S1}Wrong number lol ;)! its developer here!! Don't be so smart rotor. you can be self bombed for this wrong attempt!!Me Drexploit ~Drexploit02 github:- https://github.com/Drexploit02/ ${R0}\n"
...
printf "${S2}[${R0} 3${S2} ]${R0}>>>${S3} ABOUT TOOL ${R0}\n"
...
if [ "$options" -eq "4" ];then
  echo
  echo -e "${S2}[${S1}~${S2}]${S3} Thanks for Using Darkhack tool !${S3}"
  echo
  exit 1
else
  menux
