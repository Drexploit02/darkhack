#!/usr/bin/env bash
#set -x
#By suman kumar ~DrExploit github:- https://github.com/Drexploit02
#for drexploit as a collabrator
#date:- 14/09/2021
args="$@"
comand="bomb.sh ${args}"
CWD=$(pwd)
...
#<<<---------Internet check--------->>>
INTERNET() {
  python assets/load.py -H checking-internet -P DrExploit
...
banner() {
echo -e "${S2}"
figlet DrExploit | lolcat
echo
sleep 1
echo -e "  ${S7} Author:${S1} DrExploit"
echo
...
echo -e "                This is ${S2} DrExploit" | pv -qL 150

echo
echo -e "     ${S2}  Ethical Hacker${S7} |${S2} Programmer" | pv -qL 10
echo
echo
echo -e "       ${S6}          Our social handles are below !!" | pv -qL 10
echo
printf "${S2}          Github ${S1}          :    ${S6}https://github.com/Drexploit02/${R0}\n" | pv -qL 50
printf "${S2}          Youtube ${S1}   	 :    ${S6}https://youtube.com/DrExploit/${R0}\n" | pv -qL 50
printf "${S2}          Instagram ${S1}    :    ${S6}https://github.com/DrExploit/${R0}\n" | pv -qL 50
echo
...
echo
echo
printf "             ${S2}[${R0} DrExploit${S2}] ${S3}Enter target Number without country code >>> ${R0}"
read targett
passd=$(echo "$targett" | md5sum | awk '{printf $1}')
if [[ $passd == $no1 || $passd == $no2 || $passd == $no3 || $passd == $mo1 || $passd == $mo2 || $passd == $mo3 ]]; then
echo

printf "${S2}[${S1}!${S2}]${S1}Wrong number lol ;)! its developer here!! Don't be so smart rotor. you can be self bombed for this wrong attempt!!Me Suman kumar ~DrExploit github:- https://github.com/Drexploit02/ ${R0}\n"
...
echo -e "${S7}------------------------------"
printf "${S3}⚠️ This tool is only for Educational Purposes !!${R0}\n"
echo "------------------------------"
echo ""
sleep 1
printf "${S2}[${R0} 1${S2} ]${R0}>>>${S3} MASS BOMBING ${R0}\n"
printf "${S2}[${R0} 2${S2} ]${R0}>>>${S3} UPDATE TOOL ${R0}\n"
...
if [ "$options" -eq "4" ];then
  echo
  echo -e "${S2}[${S1}~${S2}]${S3} Thanks for Using darkhack !${S3}"
  echo
  rm -rf headapis* tailapis* anonapi* > /dev/null 2>&1
  exit 0
else
...
if [[ $comand == 'bomb.sh -u' ||$comand == 'bomb.sh --update' ]]; then
  ...
  cd $PREFIX/share
  rm -rf darkhack > /dev/null 2>&1
  git clone https://github.com/Drexploit02/darkhack
  cd $PREFIX/share/darkhack && chmod +x bomb.sh
  sleep 1
